function generateFormsFromAttendanceSlip() {
  ...
  const response = ui.alert('FORM-16,17,23', 'WOULD YOU LIKE TO GENERATE FORM-16,17,23?', ui.ButtonSet.OK_CANCEL);
  if (response !== ui.Button.OK) return; // Immediately exit if they click Cancel or the 'X'
  const startTime = Date.now();

  const config = {
    formatSpreadsheetId: '1Ta64FdPsOkH3TshNMNIOEpNURa-GWd2RR3BWIBHgsEo',
      ...
      form17: {
        columns: [
          ...
          'Wash Allowance',
          {
            type: 'formula',
            header: 'Total',
            formula: '=SUM({Basic Wages},{Payments Overtime},{Payments P.H.},{HRA},{Conve.},{Site Allowances},{Wash Allowances})'
          },
          {
            type: 'formula',
            header: 'PF',
            formula: '=MIN((SUM({Basic Wages},{Payments P.H.},{Conve.},{Site Allowances},{Wash Allowances},{FINE}))*12%, 1800)'
          },
          {
            type: 'formula',
            header: 'Others (PT)',
